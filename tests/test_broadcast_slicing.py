...
class TestBroadcastablePointsSlicing:
    ...
    def test_comparison_with_regular_points(self):
        ...
        # Create 6D data for regular Points comparison
        dat_6d = np.array([
            [0, 0, 0, 0, 10, 10],  # T=0, P=0, C=0, Z=0
            [0, 1, 0, 0, 20, 20],  # T=0, P=1, C=0, Z=0
            [1, 0, 0, 0, 30, 30],  # T=1, P=0, C=0, Z=0
            [1, 1, 0, 0, 40, 40],  # T=1, P=1, C=0, Z=0
        ])
        
        # Create 4D data for BroadcastablePoints
        dat_4d = np.array([
            [0, 0, 10, 10],  # T=0, P=0
            [0, 1, 20, 20],  # T=0, P=1
            [1, 0, 30, 30],  # T=1, P=0
        ...
        # Create slice at T=0, P=0, C=0, Z=0
        world_slice = _ThickNDSlice.make_full(
            point=(0., 0., 0., 0., 256., 256.),
            margin_left=(0.5, 0.5, 0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0.5, 0.5, 0., 0.)
        ...
        # Both should show the same result for this specific case
        assert regular_indices == [0]
        assert broadcast_indices == [0]
        
        # The key difference is that BroadcastablePoints ignores C,Z dimensions
        # This is already tested in the broadcast_slicing_behavior test above
    ...
    def test_no_broadcast_dims(self):
        ...
        dat = np.array([[0, 0, 10, 10]])
        points = BroadcastablePoints(dat, broadcast_dims=[])
        
        # Should behave like regular Points when no broadcast dims
        assert points.data.shape == (1, 4)  # No dimensions inserted
        assert points._broadcast_dims == []
    ...
    def test_various_broadcast_combinations(self, broadcast_dims):
        """Test various combinations of broadcast dimensions."""
        dat = np.array([
            [0, 0, 10, 10],
            [1, 1, 20, 20],
        ])
        ...
        # Test that slicing request can be created without errors
        world_slice = _ThickNDSlice.make_full(
