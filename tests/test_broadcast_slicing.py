...
class TestBroadcastablePointsSlicing:
    ...
    def test_slice_request_type(self):
        ...
        world_slice = _ThickNDSlice.make_full(
            point=(0., 0., 0., 0., 256., 256.),
            margin_left=(0.5, 0.5, 0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0.5, 0.5, 0., 0.)
        )
        slice_input = _SliceInput(ndisplay=2, world_slice=world_slice, order=(0, 1, 2, 3, 4, 5))
    ...
    def test_broadcast_slicing_behavior(self):
        ...
        world_slice_c5 = _ThickNDSlice.make_full(
            point=(0., 0., 5., 0., 256., 256.),  # C=5 (different from inserted 0)
            margin_left=(0.5, 0.5, 0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0.5, 0.5, 0., 0.)
        )
        slice_input_c5 = _SliceInput(ndisplay=2, world_slice=world_slice_c5, order=(0, 1, 2, 3, 4, 5))
        ...
        world_slice_z10 = _ThickNDSlice.make_full(
            point=(0., 0., 0., 10., 256., 256.),  # Z=10 (different from inserted 0)
            margin_left=(0.5, 0.5, 0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0.5, 0.5, 0., 0.)
        )
        slice_input_z10 = _SliceInput(ndisplay=2, world_slice=world_slice_z10, order=(0, 1, 2, 3, 4, 5))
        ...
        world_slice_p1 = _ThickNDSlice.make_full(
            point=(0., 1., 0., 0., 256., 256.),
            margin_left=(0.5, 0.5, 0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0.5, 0.5, 0., 0.)
        )
        slice_input_p1 = _SliceInput(ndisplay=2, world_slice=world_slice_p1, order=(0, 1, 2, 3, 4, 5))
    ...
    def test_comparison_with_regular_points(self):
        ...
        assert regular_indices == [0], f"Regular points should only show exact match, got {regular_indices}"
        assert broadcast_indices == [0, 1, 2], f"Broadcast points should show all at T=0,P=0, got {broadcast_indices}"
        ...
        world_slice_c1 = _ThickNDSlice.make_full(
            point=(0., 0., 1., 0., 256., 256.),  # C=1 now
            margin_left=(0.5, 0.5, 0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0.5, 0.5, 0., 0.)
        )
        slice_input_c1 = _SliceInput(ndisplay=2, world_slice=world_slice_c1, order=(0, 1, 2, 3, 4, 5))
        
        # Regular points - should only show index 1 (at C=1)
        data_slice = slice_input_c1.data_slice(regular_points._data_to_world.inverse)
        request = regular_points._make_slice_request_internal(slice_input_c1, data_slice)
        response = request()
        ...
        request = broadcast_points._make_slice_request_internal(slice_input_c1, data_slice)
        response = request()
        broadcast_indices_c1 = response.indices.tolist()
        
        assert regular_indices_c1 == [1], f"Regular points should only show C=1 match, got {regular_indices_c1}"
        assert broadcast_indices_c1 == [0, 1, 2], f"Broadcast points should ignore C dimension, got {broadcast_indices_c1}"
        ...
        world_slice_z1 = _ThickNDSlice.make_full(
            point=(0., 0., 0., 1., 256., 256.),  # Z=1 now
            margin_left=(0.5, 0.5, 0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0.5, 0.5, 0., 0.)
        )
        slice_input_z1 = _SliceInput(ndisplay=2, world_slice=world_slice_z1, order=(0, 1, 2, 3, 4, 5))
        
        # Regular points - should only show index 2 (at Z=1)
        data_slice = slice_input_z1.data_slice(regular_points._data_to_world.inverse)
        request = regular_points._make_slice_request_internal(slice_input_z1, data_slice)
        response = request()
        ...
        request = broadcast_points._make_slice_request_internal(slice_input_z1, data_slice)
        response = request()
        broadcast_indices_z1 = response.indices.tolist()
        
        assert regular_indices_z1 == [2], f"Regular points should only show Z=1 match, got {regular_indices_z1}"
        assert broadcast_indices_z1 == [0, 1, 2], f"Broadcast points should ignore Z dimension, got {broadcast_indices_z1}"
    ...
    def test_no_broadcast_dims(self):
        ...
        world_slice = _ThickNDSlice.make_full(
            point=(0., 0., 256., 256.),
            margin_left=(0.5, 0.5, 0., 0.),
            margin_right=(0.5, 0.5, 0., 0.)
        )
        slice_input = _SliceInput(ndisplay=2, world_slice=world_slice, order=(0, 1, 2, 3))
        
        # Get responses from both
        data_slice = slice_input.data_slice(broadcast_points._data_to_world.inverse)
        broadcast_request = broadcast_points._make_slice_request_internal(slice_input, data_slice)
        broadcast_response = broadcast_request()
        
        data_slice = slice_input.data_slice(regular_points._data_to_world.inverse)
        regular_request = regular_points._make_slice_request_internal(slice_input, data_slice)
        regular_response = regular_request()
        
        # Should have identical behavior
        assert broadcast_response.indices.tolist() == regular_response.indices.tolist() == [0]
    ...
    def test_various_broadcast_combinations(self, broadcast_dims):
        ...
        slice_input = _SliceInput(
            ...
            order=tuple(range(expected_cols))
        )
